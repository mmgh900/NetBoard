...
public class ChatTab extends Tab {
    ...
    private void sendFileMassage() {
        ...
        client.socketStreamManager.sendPacket(new Packet(massage, bytes, selectedFile.getName(), client.getClientProfile(), chat.getMembers().get(1), Packet.PacketPropose.MASSAGE));
    ...
    private void sendTextMassage() {
        ...
        if (!text.isBlank()) {
            ...
            chat.getMassages().add(massage);
            addMassageToChat(massage);
            client.socketStreamManager.sendPacket(new Packet(massage, client.getClientProfile(), chat.getMembers().get(1), Packet.PacketPropose.MASSAGE));
            client.sendProfileToServer();
        }
    }
    ...
    public void addMassageToChat(Massage massage) {
        ...
        if (massage.getMassageType().equals(Massage.MassageType.TEXT)) {
            ...
            massageSkin = new FileMassageSkin(massage, client);
        ...
        if (client.game.getGameController().chats.getSelectionModel().getSelectedItem() != null) {
            if (!client.game.getGameController().chats.getSelectionModel().getSelectedItem().equals(thisChatTab)) {
                unReadMassages++;
                client.sendProfileToServer();
                Platform.runLater(() -> setText(chat.getName() + "(" + unReadMassages + " Unread)"));
